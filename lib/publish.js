...
import { uploadToR2, deleteFromR2, getFromR2, purgeZoneCache } from '../server/lib/cloudflare.js';
import { getDb as getDbImport } from '../server/db/index.js';
import { processAndSaveImage } from '../server/lib/image-processing.js';
...
/**
 * Extract all local image paths referenced in HTML as /assets/imgs/...
 */
function extractImagePaths(html) {
  const seen = new Set();
  for (const m of html.matchAll(/\/assets\/imgs\/([\w.-]+)/g)) {
    seen.add(`assets/imgs/${m[1]}`);
  }
  return [...seen];
}

/**
 * Download external images found in HTML, save them locally via processAndSaveImage,
 * and replace external URLs with local /assets/imgs/ paths.
 */
async function downloadExternalImages(html) {
  const EXT_IMG_RE = /(?:(?:src|srcset)\s*=\s*["']|url\(\s*["']?)(https?:\/\/[^"'\s)]+\.(?:jpg|jpeg|png|webp|gif|svg))(?:["'\s)])/gi;
  const matches = [...html.matchAll(EXT_IMG_RE)];
  ...
  for (const url of uniqueUrls) {
    try {
      ...
      if (!response.ok) {
        console.warn(`  External image download FAIL (HTTP ${response.status}): ${url}`);
        continue;
      }
      ...
      const localPath = `/assets/imgs/${result.filename}`;
      urlMap.set(url, localPath);
      console.log(`  External image OK: ${url} → ${localPath}`);
    } catch (err) {
      console.warn(`  External image download FAIL: ${url}: ${err.message}`);
    }
  }

  // Replace all external URLs with their local paths
  let result = html;
  for (const [externalUrl, localPath] of urlMap) {
    result = result.replaceAll(externalUrl, localPath);
  }
...
/**
 * Generate event tracking calls for a pixel
 */
function generateEventCalls(pixel) {
  ...
  switch (pixel.type) {
    ...
    case 'taboola':
      return events
        .filter(e => e !== 'page_view')
        .map(e => `_tfa.push({notify: 'event', name: '${e}', id: ${pixel.pixel_id}});`)
        .join('\n');
...
function generatePixelCode(pixel) {
  ...
  const extraEvents = generateEventCalls(pixel);
...
<!-- End TikTok Pixel -->`;
    ...
    case 'taboola':
      return `<!-- Taboola Pixel -->
<script>
window._tfa = window._tfa || [];
window._tfa.push({notify: 'event', name: 'page_view', id: ${pixel.pixel_id}});
${extraEvents}
!function (t, f, a, x) {
...
'//cdn.taboola.com/libtrc/unip/${pixel.pixel_id}/tfa.js',
'tb_tfa_script');
</script>
<noscript>
<img src="https://trc.taboola.com/actions-handler/log/3/s2s-action?click-id=GiCIkao4MbyaJoaPFieBn2K0RVOxkAUev&name=page_view&revenue=&currency=&item-url=&quantity=0" width="0" height="0" style="display:none"/>
</noscript>
<!-- End Taboola Pixel -->`;
...
/**
 * Get pixels associated with a page via pixel_pages junction table.
 * Pixels with no page associations (page_ids empty) apply to all pages.
 * Pixels with specific page associations only apply to those pages.
 */
function getPagePixels(page, db) {
  // Get pixels explicitly linked to this page
  const linkedPixels = db.prepare(`
    SELECT p.* FROM pixels p
    INNER JOIN pixel_pages pp ON pp.pixel_id = p.id
    WHERE pp.page_id = ?
  `).all(page.id);

  // Get pixels with no page associations (global - apply to all pages)
  const globalPixels = db.prepare(`
    SELECT p.* FROM pixels p
    WHERE NOT EXISTS (SELECT 1 FROM pixel_pages pp WHERE pp.pixel_id = p.id)
  `).all();

  // Combine, deduplicating by id
  const seen = new Set();
  const result = [];
  for (const px of [...linkedPixels, ...globalPixels]) {
    if (!seen.has(px.id)) {
      seen.add(px.id);
      result.push(px);
    }
...
export async function publishPage(page, db) {
  ...
  // Normalize absolute image URLs to relative paths (fixes pages saved by copier with server origin)
  let htmlContent = page.html_content.replace(/https?:\/\/[^/'"]+\/(assets\/imgs\/[\w.-]+)/gi, '/$1');

  // Download external images and replace URLs with local paths
  const preDownloadHtml = htmlContent;
  htmlContent = await downloadExternalImages(htmlContent);

  // If external images were downloaded, persist the localized HTML back to DB
  // so GrapesJS and future publishes use local paths
  if (htmlContent !== preDownloadHtml) {
    db.prepare('UPDATE pages SET html_content = ? WHERE id = ?').run(htmlContent, page.id);
    console.log('  Updated DB html_content with localized image paths');
  }
  ...
  if (htmlContent.includes('</body>')) {
    ...
    // Fallback: append tracking script if no closing body tag
    htmlContent += `\n${trackingScript}`;
  }
  ...
  const imagePaths = extractImagePaths(finalHtml);
  ...
  for (const cfDomain of cfDomains) {
    try {
      ...
      if (cfAccount && cfDomain.r2_bucket) {
        ...
        // Upload referenced images to the same bucket
        for (const imgPath of imagePaths) {
          ...
          if (fs.existsSync(absPath)) {
            ...
            // Not found locally — check if already in domain bucket
            try {
              const r2Result = await getFromR2(cfAccount, cfDomain.r2_bucket, imgPath);
              if (r2Result) {
                imgBuffer = r2Result.buffer;
                // Cache locally for future publishes
                try {
                  const uploadsDir = path.dirname(absPath);
          ...
          try {
            await uploadToR2(cfAccount, cfDomain.r2_bucket, imgPath, imgBuffer, mimeType);
            console.log(`  R2 image OK: ${imgPath} → ${cfDomain.domain}`);
          } catch (imgErr) {
            console.error(`  R2 image FAIL: ${imgPath}: ${imgErr.message}`);
          }
        }
        // Purge edge cache for this page and its images
        if (cfDomain.cloudflare_zone_id) {
          try {
            const purgUrls = [
              `https://${cfDomain.domain}/${slug}`,
              `https://${cfDomain.domain}/`,
            ];
            for (const imgPath of imagePaths) {
              purgUrls.push(`https://${cfDomain.domain}/${imgPath}`);
            }
            await purgeZoneCache(cfAccount, cfDomain.cloudflare_zone_id, purgUrls);
            console.log(`  Cache purge OK: ${cfDomain.domain} (${purgUrls.length} URLs)`);
          } catch (purgeErr) {
            console.warn(`  Cache purge FAIL: ${cfDomain.domain}: ${purgeErr.message}`);
          }
        }
      }
    } catch (err) {

...
import { uploadToR2, deleteFromR2, getFromR2 } from '../server/lib/cloudflare.js';
...
export async function publishPage(page, db) {
  ...
  console.log(`[publish] UPLOAD_DIR=${UPLOAD_DIR}`);
  console.log(`[publish] cfDomains=${cfDomains.length}, imagePaths=${imagePaths.length}`);
  console.log(`[publish] imagePaths:`, imagePaths);
    ...
    try {
      ...
      if (cfAccount && cfDomain.r2_bucket) {
        ...
        for (const imgPath of imagePaths) {
          ...
          console.log(`  [img] ${filename}: checking local ${absPath}`);

          // Try local filesystem first
          if (fs.existsSync(absPath)) {
            imgBuffer = fs.readFileSync(absPath);
            console.log(`  [img] ${filename}: found locally (${imgBuffer.length} bytes)`);
          } else {
            console.log(`  [img] ${filename}: NOT found locally, checking domain R2...`);
            ...
            try {
              ...
              if (r2Result) {
                ...
                console.log(`  [img] ${filename}: found in domain R2 (${imgBuffer.length} bytes)`);
                ...
                console.log(`  [img] ${filename}: NOT found in domain R2 either`);
              }
            } catch (r2Err) {
              console.error(`  [img] ${filename}: R2 lookup error: ${r2Err.message}`);
            }
          }

          if (!imgBuffer) {
            console.warn(`  [img] SKIP: ${imgPath} — not found anywhere`);
            continue;
          }

          try {
            await uploadToR2(cfAccount, cfDomain.r2_bucket, imgPath, imgBuffer, mimeType);
            console.log(`  [img] UPLOADED: ${imgPath} → ${cfDomain.r2_bucket} (${imgBuffer.length} bytes)`);
          } catch (imgErr) {
            console.error(`  [img] UPLOAD FAIL: ${imgPath}: ${imgErr.message}`);
          }
        }
      }
    } catch (err) {

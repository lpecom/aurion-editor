...
import { uploadToR2, deleteFromR2, getFromR2, getDefaultCloudflareAccount, R2_IMAGES_BUCKET } from '../server/lib/cloudflare.js';
      ...
      if (cfAccount && cfDomain.r2_bucket) {
        ...
        for (const imgPath of imagePaths) {
          ...
          if (fs.existsSync(absPath)) {
            ...
            // Not found locally (e.g. Railway ephemeral) — try multiple R2 sources
            const centralAccount = getDefaultCloudflareAccount();
            if (centralAccount) {
              // 1. Try central aurion-assets bucket
              try {
                const r2Result = await getFromR2(centralAccount, R2_IMAGES_BUCKET, filename);
                if (r2Result) imgBuffer = r2Result.buffer;
              } catch (_) { /* non-fatal */ }

              // 2. Try the domain's own bucket (might have it from a previous publish)
              if (!imgBuffer) {
                try {
          ...
          if (!imgBuffer) {
            console.warn(`  Image not found (local or R2), skipping: ${imgPath}`);
            continue;
          }

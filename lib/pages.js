...
export function injectPixels(html, pixels) {
  ...
  const pixelCodes = pixels.map(pixel => {
    ...
    switch (pixel.type) {
      ...
      case 'google':
        return `<script async src="https://www.googletagmanager.com/gtag/js?id=${pixel.pixel_id}"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${pixel.pixel_id}');</script>`;
      case 'tiktok':
